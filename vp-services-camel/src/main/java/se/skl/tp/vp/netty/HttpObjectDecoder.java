 ...
 * under the License.
 *
 * Modifications copyright (C) 2019 Inera AB
 * The intention of the modifications is to handle servers responses with a
 * greater content-length header then the actual content.
 * The modifications in this file applies is made in the decodeLast method where
...
public abstract class HttpObjectDecoder extends ByteToMessageDecoder {
    ...
    protected void decodeLast(ChannelHandlerContext ctx, ByteBuf in, List<Object> out) throws Exception {
        ...
        if (message != null) {
            ...
            // Added READ_FIXED_LENGTH_CONTENT to be nice to servers responding with a
            // greater content-length then the content really is (gerkstam)
            if ( (currentState == State.READ_VARIABLE_LENGTH_CONTENT || currentState == State.READ_FIXED_LENGTH_CONTENT) && !in.isReadable() && !chunked) {
                // End of connection.
                out.add(LastHttpContent.EMPTY_LAST_CONTENT);
                resetNow();
                return;
