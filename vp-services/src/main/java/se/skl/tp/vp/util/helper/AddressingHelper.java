...
public class AddressingHelper extends VPHelperSupport {
	...
	public String getAddress() {
		 ...
		 */
		final List<VirtualiseringsInfoType> services = this.getAllVirtualizedServices(this.agent.visaVagval(request), input);
	...
	private VagvalInput createRequestToServiceDirectory() {
		...
		vagvalInput.serviceNamespace = VPUtil.extractNamespaceFromService((QName) this.getMuleMessage().getProperty(VPUtil.SERVICE_NAMESPACE));
	...
	private void validateRequest(final VagvalInput request) {
		if (this.getLog().isDebugEnabled()) {
			this.getLog().debug("Calling vagvalAgent with serviceNamespace:"
	...
	private List<VirtualiseringsInfoType> getAllVirtualizedServices(final VisaVagvalResponse response, final VagvalInput request) {
		...
		if (this.getLog().isDebugEnabled()) {
			...
			for (VirtualiseringsInfoType vvInfo : virtualiseringar) {
				this.getLog().debug("VagvalAgent response item RivProfil: " + vvInfo.getRivProfil()
