...
public class PingForConfigurationProducerRivTa21 implements PingForConfigurationResponderInterface {
  ...
  private static final Logger log = LoggerFactory
      .getLogger(PingForConfigurationProducerRivTa21.class);
  ...
  public PingForConfigurationResponseType pingForConfiguration(String logicalAddress,
    ...
    int authInfoSize = vagvalAgent.getNumberOfBehorigheter();
    int routingInfoSize = vagvalAgent.getNumberOfVagval();

    if (log.isDebugEnabled()) {
      log.debug(
          "pingForConfiguration result, nr of authorizations:{}, nr of routing information:{}",
          authInfoSize, routingInfoSize);
    }

    if (!resourcesNeededForVpAvailable()) {
      log.error("Severe problem, vp reports needed resources are missing, routing size: {}",
          routingInfoSize);
      log.error("Severe problem, vp reports needed resources are missing, authorization size: {}",
          authInfoSize);
      throw new RuntimeException(VpSemanticErrorCodeEnum.VP012
          + " Severe problem, vp does not have all necessary reources to operate");
    }

    response.getConfiguration()
        .add(createConfiguration("Applikation", rb.getString("APPLICATION_NAME")));
    response.getConfiguration()
        .add(createConfiguration("Anropsbehörigheter", String.valueOf(authInfoSize)));
    response.getConfiguration()
        .add(createConfiguration("Logiska-adresser", String.valueOf(routingInfoSize)));
  ...
  boolean resourcesNeededForVpAvailable() {
    return vagvalAgent != null
        && vagvalAgent.getNumberOfBehorigheter() > 0
        && vagvalAgent.getNumberOfVagval() > 0;
  }
